...
module.exports = {
  ...
  info: "Manage and enforce banned words",
  category: "members",
...
noPrefix: async function ({ api, event }) {
 ...
 letStart: async function({ api, event, target, message }) {
  ...
  const Admin = (await api.getThreadInfo(threadID)).adminIDs.some(idInfo => idInfo.id === api.getCurrentUserID());
  const isUserAdmin = (await api.getThreadInfo(threadID)).adminIDs.some(idInfo => idInfo.id === event.senderID);
  if (!Admin || !isUserAdmin) {
    return api.sendMessage("🛡️ | You're not able to use Badwords commands because you are not an admin this group chats", threadID, messageID);
  }
  ...
  switch (action) {
    case 'add':
      ...
      fs.writeFileSync(path.join(__dirname, `../commands/noprefix/${threadID}.json`), JSON.stringify(bannedWords[threadID]), "utf8");
